 ...
 *
 * playController - Matches users who want to play the same sport.
 */
...
angular.module('dashboard.controllers').controller('playController', ['$scope', '$http', '$rootScope', function ($scope, $http, $rootScope) {
    ...
    /* Grab the users sports that are marked as their "favourites" */
    $http({
        method: 'POST',
    ...
    /* Get all the sports in the database created by an Admin */
    ...
    $scope.acceptChallenge = function (item) {
        ...
        $http.get("api/queue/" + item.queue_id).success(function (data) {
            ...
            //Creates a match object to keep track of matching status
            $http.post("api/match", {
                sport_id: $scope.sport.id,
