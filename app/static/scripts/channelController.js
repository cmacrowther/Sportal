 ...
 *
 * channelController - Houses functions for the messaging pages including: creating conversation
 * channels, creating Direct message convos, loading past conversations, sending messages, &
 * deleting messages.
 *
 * Initializes the pusher plugin for real-time messaging.
 */
...
angular.module('dashboard.controllers').controller('channelController', ['$scope', '$rootScope', '$http', '$routeParams', '$timeout', function ($scope, $rootScope, $http, $routeParams, $timeout) {
    ...
    if ($routeParams.user_id) {
        $http.get("api/user/" + $routeParams.user_id).success(function (data) {
            //the user exists
        ...
        $http.get("api/team/" + $routeParams.team1_id).success(function (data) {
            $scope.first_team = data;
            $http.get("api/team/" + $routeParams.team2_id).success(function (data) {
    ...
    /* Gets users from database so that we can populate a
     * list with users to start a conversation with */
    $http.get("/api/user").success(function (data) {
        $scope.users = data.objects;
    });

    /* Toggles between delete view & regular view */
    $scope.editConvos = function () {
        $scope.editable = !$scope.editable;
    };

    /* After clicking on a conversation - move to the conversation view.
     * Sets convo title and loads past messages from convo. */
    $scope.setConversation = function (conversation_id) {
        ...
        $http({
            ...
            $http.get("api/conversation/" + conversation_id).success(function (data) {
                if (data.user_one == $rootScope.userObject.id) {
                    $scope.convo_with = data.user_two;
                    $http.get("/api/user/" + data.user_two).success(function (data) {
                        $scope.title = "Conversation with " + data.first_name + " " + data.last_name;
                    })
                }
                else {
                    $scope.convo_with = data.user_one;
                    $http.get("/api/user/" + data.user_one).success(function (data) {
                        $scope.title = "Conversation with " + data.first_name + " " + data.last_name;
                    })
                }
            });
    ...
    /* Creates a new convo so that a user can begin sending messages
     * to another user */
    $scope.createConversation = function (user_id) {
        if (user_id == $rootScope.userObject.id) {
            ...
            var flag = 0; //flag if already in convo with user
            ...
            if (flag == 0) {
                ...
                $http.post("api/conversation", {
                    ...
                    $http.get("api/user/" + user_id).success(function (user) {
                        var convo = {
                            id: data.id,
    ...
    /* tells angular if the user is currently in a message pane, in a
     * channel or in a direct message convo */
    ...
    /* Used to send message, triggers pusher in back-end for sending
     * message and increasing counter */
    ...
    $scope.setChannel = function (channel_id) {
        ...
        $http.get("/api/channel/" + channel_id).success(function (data) {
    ...
    /* Passes the info required to delete to the delete conversation Modal */
    $scope.setModal = function (item) {
        $scope.modalObject = item;
    };

    /* Deletes DIRECT message conversations */
    ...
    /* Sets a conversation to having been read when a convo is selected
     * so that the user can keep track of which messages they have seen */
        ...
        $http({
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i].user_id != $rootScope.userObject.id && data[i].is_read_user_two == 0) {
                    //They didn't send the message and it's unread, therefore is_read_user_two needs to change to READ
                    var is_read = {
                        id: data[i].uhm_id,
